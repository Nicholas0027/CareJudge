...
\end{aligned}
    \label{eq:features}
\end{equation}
The three protocol-stability families (position, rubric, self-consistency) constitute the paper's contributed signal; the confidence and length families are standard baselines included so the calibrator can exploit them where informative. We deliberately \emph{exclude} the simulated-annotator signal from the feature vector---reproducing Trust-or-Escalate's few-shot personas is orthogonal to our hypothesis, and a disabled ($N{=}0$) constant feature would only add a spurious degree of freedom---and instead report a dedicated Trust-or-Escalate baseline in the head-to-head comparison (Section~\ref{sec:exp:comparison}), with a leakage-controlled construction (demonstrations from $D_{\text{train}}$ only) described in Appendix~\ref{app:repro}.
